 <?php
    ...
    if($count2==1){
                        echo'<a href="/ApplicationTracker/src/hod/hod_pend_stud.php"><div class="ui active step">
    ...
    if($result1){
        $init=0;
         while($info=mysqli_fetch_array($result1)){
            echo '<tr class="clickable-row" data-href="/ApplicationTracker/src/hod/hod_application.php?roll_no='.$info['roll_no'].'&strdate='.$info['strdate'].'">
                      <td>'.$info['roll_no'].'</td>
                      <td>'.$info['strdate'].'</td>
                      <td>'.$info['purpose'].'</td>
